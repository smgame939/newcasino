<?php

include_once('../class/mobiledetect.class.php');
$mdetect = new MobileDetect();

if ($mdetect->isMobile()) {
?>
    <!-- Mobile -->
    <div class="vfm vfm--inset vfm--fixed" style="z-index: 1006;">
        <div class="vfm__overlay vfm--overlay vfm--absolute vfm--inset"></div>
        <div class="vfm__container vfm--absolute vfm--inset vfm--outline-none model-main popup-signup model-open" aria-expanded="true" role="dialog" aria-modal="true" tabindex="-1" enter-class="vue-modal-enter-from" leave-class="vue-modal-leave-from" style="">
            <div class="vfm__content model-inner modal-fade-in">
                <div class="model-wrap">
                    <div class="pop-up-content">
                        <div class="signup-content">
                            <div class="close-btn" id="closePop"><span class="icon-close"></span></div>
                            <div class="input-area">
								<form id="frm12">
                                    <div class="input-list">
                                        <div class="list list-id">
                                            <div class="txt"><span class="icon-id"><em>*</em></span></div><input type="text" name="iu_id" id="iu_id" placeholder="아이디 (영문, 숫자 포함 4자 이상)" class="">
                                            <div class="checkUse undefined-member"></div>
                                        </div><button class="check-button idcheck" ref="idck" type="button">중복</button><!---->
                                        <div class="alarm a_id" style="display: none;">계정등록이 가능합니다.</div>
                                    </div>
									<div class="input-list">
                                        <div class="list list-id">
                                            <div class="txt"><span class="icon-id"><em>*</em></span></div><input type="text" name="iu_nick" id="iu_nick" placeholder="닉네임" class="">
                                            <div class="checkUse undefined-member"></div>
                                        </div><button class="check-button nccheck" ref="ncck" type="button">중복</button><!---->
                                        <div class="alarm a_nick" style="display: none;">계정등록이 가능합니다.</div>
                                    </div>
                                    <div class="input-list">
                                        <div class="list full-list">
                                            <div class="txt"><span class="icon-pw"><em>*</em></span></div><input type="password" placeholder="비밀번호 (영문, 숫자 포함 6자이상)" class="" name="iu_pw1" id="iu_pw1">
                                            <div class="checkUse"></div>
                                        </div>
                                        <div class="rule-hint" style="display: none;">
                                            <div class="txt-hint">비밀번호는 아래의 조건이 포함되어야 합니다 :</div>
                                            <ul>
                                                <li class="">
                                                    <div class="checkUse"></div>
                                                    <p>최소 6자, 최대 16자</p>
                                                </li>
                                                <li class="">
                                                    <div class="checkUse"></div>
                                                    <p>영숫자 조합</p>
                                                </li>
                                                <li class="">
                                                    <div class="checkUse"></div>
                                                    <p>(선택)특수기호!@#$%^&amp;*()+_</p>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                    <div class="input-list">
                                        <div class="list full-list">
                                            <div class="txt"><span class="icon-pw"><em>*</em></span></div><input type="password" placeholder="비밀번호 확인 (영문, 숫자 포함 6자이상)" class="" name="iu_pw2" id="iu_pw2">
                                            <div class="checkUse"></div>
                                        </div><!---->
                                    </div>
                                    <div class="input-list">
                                        <div class="list full-list">
                                            <div class="txt"><span class="icon-pw"><em>*</em></span></div><input type="password" pattern="[0-9]*" inputmode="numeric" placeholder="출금 비밀번호 ( 4-6자리, 숫자만 가능 )" class="" name="iu_cashcode" id="iu_cashcode">
                                            <div class="checkUse"></div>
                                        </div><!---->
                                    </div>
                                    <div class="input-list">
                                        <div class="list full-list">
                                            <div class="txt"><span class="icon-phone"><em>*</em></span></div><input type="text" pattern="[0-9]*" inputmode="numeric" placeholder="휴대폰번호 (숫자만 입력가능)" class="" name="iu_phone1" id="iu_phone1"><!---->
                                            <div class="checkUse"></div>
                                        </div><!---->
                                    </div>
                                    <div class="input-list input-birth">
                                        <div class="list full-list">
                                            <div class="txt"><span class="icon-calendar"><em>*</em></span></div>
                                            <div class="license-number">
                                                <div class="number-box"><input type="text" pattern="[0-9]*" inputmode="numeric" class="input-one" placeholder="예)19760325" name="birthday"></div>
                                            </div>
                                            <div class="checkUse"></div>
                                        </div><!---->
                                    </div>
                                    <div class="input-list input-birth">
										<div class="list full-list">
											<div class="txt"><span class="icon-gender"><em>*</em></span></div>
											<div class="license-number">
												<div class="number-box">
													<ul class="sex">
														<li><input id="male" type="radio" name="sex" value="true"><label for="male">남</label></li>
														<li><input id="female" type="radio" name="sex" value="false"><label for="female">여</label></li>
													</ul>
												</div>
											</div>
											<div class="checkUse"></div>
										</div>
									</div>

                                    <div class="line"></div>
                                    <div class="input-list">
                                        <div class="list icon-black full-list">
                                            <div class="txt"><span class="icon-id"><em>*</em></span></div><input type="text" placeholder="예금주 (추후 변경 불가)" class="" name="iu_name" id="iu_name">
                                            <div class="checkUse"></div>
                                        </div><!---->
                                    </div>
                                    <div class="input-list">
                                        <div class="list icon-black full-list">
                                            <div class="txt"><span class="icon-bank-s"><em>*</em></span></div><input type="text" class="bank-btn" placeholder="입금은행 선택" readonly="" name="iu_bank" id="chooseBank_m">
                                            <div class="checkUse"></div>
                                        </div><!---->
                                    </div>
                                    <div class="input-list">
                                        <div class="list icon-black full-list">
                                            <div class="txt"><span class="icon-bank"><em>*</em></span></div><input type="text" pattern="[0-9]*" inputmode="numeric" placeholder="계좌 번호(숫자만 입력)" class="" name="iu_booknum">
                                            <div class="checkUse"></div>
                                        </div><!---->
                                    </div>
                                    <div class="input-list">
                                        <div class="list icon-black full-list">
                                            <div class="txt"><span class="icon-recommend"><em style="display: none;">*</em></span></div><input type="password" placeholder="가입코드를 입력하세요" class="" name="iu_ref"><input type="text" disabled="" style="display: none;">
                                            <div class="checkUse"></div>
                                        </div><!---->
                                    </div>
                                    <div class="bgbtn-box"><button class="signup-button _reg_btn"  ref="frm12" type="button">
                                            <div>회원가입</div>
                                        </button><button class="cancel-button" type="button" onclick="$('#closePop').click()">
                                            <div>취소</div>
                                        </button></div>
                                </form>
                            </div>
                        </div>
                    </div>
                </div><!--v-if-->
            </div>
        </div>
    </div>
<?php } else { ?>
    <!-- Desktop -->
    <div class="vfm vfm--inset vfm--fixed" style="z-index: 1000;"><!--v-if-->
        <div class="vfm__container vfm--absolute vfm--inset vfm--outline-none NAV_reg" aria-expanded="true" role="dialog" aria-modal="true" tabindex="-1" style="">
            <div class="vfm__content pupop_content pupop_registered fadeIn animated">
                <h5 class="XX" id="closePop"><span class="icon-iconCross"></span></h5>
                <div class="reg relative backface-hidden">
                    <div class="topFrame">
                        <div class="top"><img src="/assets/image/logo-6995bb1c.svg"></div>
                        <div class="title">
                            <h2>회원가입</h2>
                        </div>
                    </div>
                    <form id="frm12">
                        <div class="upFrame">
                            <div class="up">
                                <div class="input_zone">
                                    <div class="leftZone lineUse">
                                        <div class="input-Out-all">
                                            <div class="input-out-frame verifiUse">
                                                <div class="inputFrame verifiUse">
                                                    <div class="info"><span class="ficon icon-icconMan"></span>
                                                        <div class="txt">ID<span>*</span></div>
                                                    </div>
                                                    <div class="input-inner-all">
                                                        <div class="input_content btnUse"><input type="text" name="iu_id" id="iu_id" placeholder="아이디 (영문, 숫자 포함 4자 이상)" class="">
                                                            <div class="checkUse ko-KR-member"><button type="button" class="regBtn blueB idcheck" ref="idck">중복</button></div><!---->
                                                            <div class="hint" style="display: none;" >
                                                                <div class="txt"></div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
											<div class="input-out-frame verifiUse">
                                                <div class="inputFrame verifiUse">
                                                    <div class="info"><span class="ficon icon-icconMan"></span>
                                                        <div class="txt">닉네임<span>*</span></div>
                                                    </div>
                                                    <div class="input-inner-all">
                                                        <div class="input_content btnUse"><input type="text" name="iu_nick" id="iu_nick" placeholder="닉네임" class="">
                                                            <div class="checkUse ko-KR-member"><button type="button" class="regBtn blueB nccheck" ref="ncck">중복</button></div><!---->
                                                            <div class="hint" style="display: none;" >
                                                                <div class="txt"></div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="input-Out-all">
                                            <div class="input-out-frame remindUse">
                                                <div class="inputFrame verifiUse">
                                                    <div class="info"><span class="ficon icon-icconPW"></span>
                                                        <div class="txt">비밀번호<span>*</span></div>
                                                    </div>
                                                    <div class="input-inner-all">
                                                        <div class="input_content"><input type="password" placeholder="비밀번호 (영문, 숫자 포함 6자이상)" class="" name="iu_pw1" id="iu_pw1"></div>
                                                        <div class="remind-msg-zone" style="display: none;">
                                                            <h5>비밀번호는 아래의 조건이 포함되어야 합니다 :</h5>
                                                            <div class="checkFrame">
                                                                <div class="checkZone">
                                                                    <div class="dot"></div>
                                                                    <h5>최소 6자, 최대 16자</h5>
                                                                </div>
                                                                <div class="checkZone">
                                                                    <div class="dot"></div>
                                                                    <h5>영숫자 조합</h5>
                                                                </div>
                                                                <div class="checkZone">
                                                                    <div class="dot"></div>
                                                                    <h5>(선택)특수기호!@#$%^&amp;*()+_</h5>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
											
											<div class="input-out-frame verifiUse">
                                                <div class="inputFrame verifiUse">
                                                    <div class="info"><span class="ficon icon-icconMOBILE"></span>
                                                        <div class="txt">휴대폰 번호<span>*</span></div>
                                                    </div>
                                                    <div class="input-inner-all">
                                                        <div class="input_content"><input type="text" placeholder="숫자만 입력가능" class="" name="iu_phone1" id="iu_phone1">
                                                            <div class="checkUse"><!----></div><!---->
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            
                                        </div>
                                        <div class="input-Out-all">
                                            <div class="input-out-frame">
                                                <div class="inputFrame verifiUse">
                                                    <div class="info"><span class="ficon icon-icconPW"></span>
                                                        <div class="txt">비밀번호 확인<span>*</span></div>
                                                    </div>
                                                    <div class="input-inner-all">
                                                        <div class="input_content"><input type="password" placeholder="비밀번호 확인 (영문, 숫자 포함 6자이상)" class="" name="iu_pw2" id="iu_pw2"><!----></div>
                                                    </div>
                                                </div>
                                            </div>
											<div class="input-out-frame verifiUse">
                                                <div class="inputFrame verifiUse">
                                                    <div class="info"><span class="ficon icon-icconPW"></span>
                                                        <div class="txt">출금 비밀번호<span>*</span></div>
                                                    </div>
                                                    <div class="input-inner-all">
                                                        <div class="input_content"><input type="password" placeholder="출금 비밀번호 ( 4-6자리, 숫자만 가능 )" class="" name="iu_cashcode" id="iu_cashcode"><!----></div>
                                                    </div>
                                                </div>
                                            </div>
                                            
                                        </div>
                                       
                                        <div class="input-Out-all">
                                            <div class="input-out-frame">
                                                <div class="inputFrame verifiUse noLine">
                                                    <div class="info"><span class="ficon icon-icconCALENDAR"></span>
                                                        <div class="txt">생년월일<span></span></div>
                                                    </div>
                                                    <div class="input-inner-all">
                                                        <div class="input_content"><input type="text" placeholder="예)19760325" class="" name="birthday"><!----></div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="input-out-frame">
                                                <div class="inputFrame noLine gender">
                                                    <div class="info"><span class="icon-icconGendar icon-icon-icconGendar ficon"></span>
                                                        <div class="txt">성별<span></span></div>
                                                    </div>
                                                    <div class="input-inner-all">
                                                        <div class="input_content"><input type="text" placeholder="">
                                                            <ul class="sex">
                                                                <li class="male"><input id="male" type="radio" name="sex" value="true"><label for="male">남</label></li>
                                                                <li class="female"><input id="female" type="radio" name="sex" value="false"><label for="female">여</label></li>
                                                            </ul><!---->
                                                            <div class="checkUse"></div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="input-Out-all">
                                            <div class="input-out-frame">
                                                <div class="inputFrame verifiUse">
                                                    <div class="info"><span class="ficon icon-icconMan"></span>
                                                        <div class="txt">예금주<span>*</span></div>
                                                    </div>
                                                    <div class="input-inner-all">
                                                        <div class="input_content"><input type="text" placeholder="예금주 (추후 변경 불가)" class="" name="iu_name" id="iu_name"><!----></div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="input-out-frame">
                                                <div class="inputFrame verifiUse">
                                                    <div class="info"><span class="ficon icon-icconBANK"></span>
                                                        <div class="txt">은행이름<span>*</span></div>
                                                    </div>
                                                    <div class="input-inner-all">
                                                        <div class="input_content btnUse01"><input type="text" placeholder="입금은행 선택"  class="" name="iu_bank" id="iu_bank">
                                                            <div class="checkUse ko-KR-choose"><button id="chooseBank" type="button" class="regBtn goBank blueB">선택</button></div><!---->
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="input-Out-all">
                                            <div class="input-out-frame">
                                                <div class="inputFrame verifiUse">
                                                    <div class="info"><span class="ficon icon-icconPC"></span>
                                                        <div class="txt">계좌번호<span>*</span></div>
                                                    </div>
                                                    <div class="input-inner-all">
                                                        <div class="input_content"><input type="text" placeholder="계좌 번호(숫자만 입력)" class="" name="iu_booknum" id="iu_booknum"><!----></div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="input-out-frame">
                                                <div class="inputFrame verifiUse">
                                                    <div class="info"><span class="ficon icon-icconRECOMMEND"></span>
                                                        <div class="txt">가입코드<span style="display: none;">*</span></div>
                                                    </div>
                                                    <div class="input-inner-all">
                                                        <div class="input_content"><input type="password" placeholder="가입코드를 입력하세요" class="" name="iu_ref" id="iu_ref"><input type="text" disabled="" style="display: none;"><!----></div>
                                                        <!--<div class="remind-txt">*가입코드 미입력 시 본사코드로 가입됩니다.</div>-->
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="btnFrame"><button type="button" class="btn_p disableBtn _reg_btn"  ref="frm12">회원가입</button>
                                    <div class="txtZone">
										<a></a>
										<a href="javascript:void(0)" class="goLogin">이미 계정이 있습니까?<span onclick="$('#loginBtn').click()">여기에서 로그인 하기</span></a>
									</div>
                                </div>
                            </div>
                        </div>
                    </form>
                </div><!--v-if-->
            </div>
        </div>
    </div>

<?php } ?>

<!-- Bank List Modal PC -->
<div id="bank-list" class="vfm vfm--inset vfm--fixed" style="display:none;z-index: 1002;"><!--v-if-->
    <div class="vfm__container vfm--absolute vfm--inset vfm--outline-none NAV_bank" aria-expanded="true" role="dialog" aria-modal="true" tabindex="-1" style="">
        <div class="vfm__content pupop_content pupop_registered fadeIn animated">
            <div class="reg">
                <h5 class="XX closeBankList"><span class="icon-iconCross"></span></h5>
                <div class="title">
                    <h2>입금은행 선택</h2>
                </div>
                <div class="up">
                    <div class="btns">
                        <ul>
                            <li class="idnFrame fadeInUp animated p-4px bank01">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">카카오뱅크</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank02">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">농협은행</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank03">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">국민은행</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank04">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">신한은행</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank06">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">우리은행</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank08">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">새마을금고</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank09">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">하나은행</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank10">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">부산은행</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank12">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">우체국</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank14">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">SC제일은행</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank16">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">광주은행</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank18">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">전북은행</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank19">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">제주은행</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                            <li class="idnFrame fadeInUp animated p-4px bank22">
                                <div class="logo logoA aniLogo03"></div>
                                <div class="txt01 w-100% text-center max-w-100% mt-4px"><span class="bnkName">한국씨티은행</span></div>
                                <div class="mask">
                                    <div class="inner"></div>
                                </div>
                            </li>
                        </ul>
                    </div>
                </div>
            </div><!--v-if-->
        </div>
    </div>
</div>

<!-- Bank List Modal Mobile -->
<div id="bank-list-m" class="vfm vfm--inset vfm--fixed" style="display:none;z-index: 1010;">
    <div class="vfm__overlay vfm--overlay vfm--absolute vfm--inset"></div>
    <div class="vfm__container vfm--absolute vfm--inset vfm--outline-none model-main popup-bank popup-bank-open model-open" aria-expanded="true" role="dialog" aria-modal="true" tabindex="-1" enter-class="vue-modal-enter-from" leave-class="vue-modal-leave-from" style="">
        <div class="vfm__content model-inner">
            <div class="model-wrap">
                <div class="pop-up-content">
                    <div class="head-top">
                        <div class="title-name">입금은행 선택</div>
                        <div class="close-btn bank-close-btn closeBankList-m"><span class="icon-close"></span></div>
                    </div>
                    <div class="bank-content">
                        <ul>
                            <li class="idnFrame fadeInUp animated bank01"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="카카오뱅크"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">카카오뱅크</span></a></li>
                            <li class="idnFrame fadeInUp animated bank02"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="농협은행"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">농협은행</span></a></li>
                            <li class="idnFrame fadeInUp animated bank03"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="국민은행"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">국민은행</span></a></li>
                            <li class="idnFrame fadeInUp animated bank04"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="신한은행"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">신한은행</span></a></li>
                            <li class="idnFrame fadeInUp animated bank06"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="우리은행"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">우리은행</span></a></li>
                            <li class="idnFrame fadeInUp animated bank08"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="새마을금고"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">새마을금고</span></a></li>
                            <li class="idnFrame fadeInUp animated bank09"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="하나은행"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">하나은행</span></a></li>
                            <li class="idnFrame fadeInUp animated bank10"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="부산은행"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">부산은행</span></a></li>
                            <li class="idnFrame fadeInUp animated bank12"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="우체국"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">우체국</span></a></li>
                            <li class="idnFrame fadeInUp animated bank14"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="SC제일은행"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">SC제일은행</span></a></li>
                            <li class="idnFrame fadeInUp animated bank16"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="광주은행"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">광주은행</span></a></li>
                            <li class="idnFrame fadeInUp animated bank18"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="전북은행"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">전북은행</span></a></li>
                            <li class="idnFrame fadeInUp animated bank19"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="제주은행"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">제주은행</span></a></li>
                            <li class="idnFrame fadeInUp animated bank22"><a href="javascript:void(0)" class="min-h-78px! flex-center! frame" aria-label="한국씨티은행"><img src="data:image/png;base64," alt=""><span class="w-100% text-center max-w-100% mt-4px">한국씨티은행</span></a></li>
                        </ul>
                    </div>
                </div>
            </div><!--v-if-->
        </div>
    </div>
</div>